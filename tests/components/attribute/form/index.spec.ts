...
describe("AttributeForm component", () => {
  ...
  it("renders unchanged", async () => {
    const container = await renderSuspended(AttributeForm, commonOptions);
    ...
    expect(container.html()).toMatchInlineSnapshot(`
      "<div id="test-wrapper">
        <form novalidate="" class="flex flex-col gap-4">
          ...
          </div><button type="submit" class="bg-primary hover:bg-primary/90 text-white font-semibold inline-flex items-center gap-x-2 leading-tight justify-center rounded px-8 py-2 text-sm bg-primary hover:bg-primary/90 text-white">test</button><button type="button" class="text-white bg-secondary hover:bg-secondary/90 font-semibold inline-flex items-center gap-x-2 leading-tight justify-center rounded px-8 py-2 text-sm text-white bg-secondary hover:bg-secondary/90"> Resetuj </button>
        </form>
      </div>"
    `);
  });
