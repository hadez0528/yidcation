...
import AttributeBox from "@@/components/attribute/Box.vue";

describe("AttributeBox component", () => {
  it("renders unchanged", async () => {
    const container = await renderSuspended(AttributeBox);

    expect(container.html()).toMatchInlineSnapshot(`
      "<div id="test-wrapper">
        <div class="bg-white rounded p-4 uppercase pb-0 h-min pb-0 h-min">
          <div class="flex flex-row align-start justify-between">
            <h3 class="text-xl text-secondary font-semibold">Atrybuty</h3><button class="bg-primary hover:bg-primary/90 text-white font-semibold inline-flex items-center gap-x-2 leading-tight justify-center rounded px-8 py-2 text-sm bg-primary hover:bg-primary/90 text-white">Dodaj atrybut</button>
          </div>
